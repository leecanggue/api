...
public class GradeScenario {
	/*
	 ...
	 1. VO, Interface 클래스 다이어그램을 받아든다. 본인이 작성햇던 오더를 받았건.
	 일단 본인이 작성하지 않아서 잘 모르는 상황이라는 설정.
	 혹은 본인이 하더라도 짐작가는 대로 설정한 기능이라는 가정하에.
	 2. VO getter, setter ..implement 를 일반 구현한다.
	 ...
	 대출 예를 들면
		 1-input () // 학생별로 성적을 입력받는 것인가.. 그럼 입력받아서
		 			저장할 공간이 필요하겠는데... 배열이 다중 변수를 저장하는 공간인데 한번 생성하면 추가로 늘릴수 없으니 동적으로
		 			확장하는 컬렉션을 선택해야겠군. 그런데 내가 지금 아는건 밷터 뿐이니 백터에 넣어야지.
