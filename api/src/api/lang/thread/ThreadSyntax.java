...
/*
Process : 특정 기능을 목표 이루어진 함수, 프로시저, 메소드 집합
...
-- 객체지향적인 방법은 인터페이스를 구현하는 방법을 취하는것이다.
 */
/*
스레드 콜백 메소드
- start() : JVM 이 스케줄상 여유가 생겼을때, 스레드 스케줄상 run() 이 호출된다.
- run() : 스레드 상태에서 처리하려고 하는 모든 비즈니스 로직이 구현됨
- sleep(long second) : 지정된 시간동안 스레드를 쉬게 하고, 그시간이 지나면
	다시 작동한다. 1000 분의 1초
- wait() : 현제 스레드 무한대기. notify(), notifyAll() 메소드로 재실행 시킴
- suspend() : 일시정지. 재실행은 resume();
- yield() : 스레드 실행 권한을 다른 스레드에 넘김
- stop() : 스레드 실행을 종료
 */
/*
단일 스레드
